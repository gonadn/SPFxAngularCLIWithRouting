 ...
 */
class ChangeManager {
    constructor(_rushConfiguration, _lockStepProjectsToExclude) {
        this._rushConfiguration = _rushConfiguration;
        this._lockStepProjectsToExclude = _lockStepProjectsToExclude;
    }
    /**
    ...
    load(changesPath, prereleaseToken = new PrereleaseToken_1.default(), includeCommitDetails = false) {
        ...
        this._allChanges = PublishUtilities_1.default.findChangeRequests(this._allPackages, this._changeFiles, includeCommitDetails, this._prereleaseToken, this._lockStepProjectsToExclude);
    ...
    validateChanges(versionConfig) {
        Object
            ...
            if (projectInfo) {
                const versionPolicyName = projectInfo.versionPolicyName;
                if (versionPolicyName) {
                    const changeInfo = this._allChanges[key];
                    const versionPolicy = versionConfig.getVersionPolicy(versionPolicyName);
    ...
    apply(shouldCommit) {
        ...
        const updatedPackages = PublishUtilities_1.default.updatePackages(this._allChanges, this._allPackages, shouldCommit, this._prereleaseToken, this._lockStepProjectsToExclude);
        return updatedPackages;
    }
    updateChangelog(shouldCommit, updatedPackages) {
        ...
        if (!this._prereleaseToken.hasValue) {
            // Update changelogs.
            const updatedChangelogs = ChangelogGenerator_1.default.updateChangelogs(this._allChanges, this._allPackages, shouldCommit);
